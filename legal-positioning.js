  ...
  const pages = {
    privacy: {
      en: {
        ...
        privacy_analytics: 'For the public Veylto Labs website, analytics are optional. Only after explicit analytics consent, a lightweight visitor counter sends the website domain, page path, page title and browser timezone to a Google Apps Script endpoint used for aggregated visit statistics. Session storage is used to avoid counting more than one visit per browser session. No advertising tracking is enabled.',
      ...
      it: {
        ...
        privacy_analytics: 'Per il sito pubblico Veylto Labs, gli analytics sono facoltativi. Solo dopo un consenso esplicito agli analytics, un contatore visite leggero invia dominio del sito, percorso pagina, titolo pagina e fuso orario del browser a un endpoint Google Apps Script usato per statistiche aggregate sulle visite. La session storage evita di conteggiare più di una visita per sessione del browser. Non è attivo alcun tracciamento pubblicitario.',
      ...
      es: {
        ...
        privacy_analytics: 'En el sitio público de Veylto Labs, la analítica es opcional. Solo tras un consentimiento explícito, un contador ligero envía el dominio del sitio, la ruta y el título de la página y la zona horaria del navegador a un endpoint de Google Apps Script utilizado para estadísticas agregadas de visitas. El almacenamiento de sesión evita contar más de una visita por sesión del navegador. No se activa seguimiento publicitario.',
      ...
      fr: {
        ...
        privacy_analytics: 'Pour le site public Veylto Labs, l’analyse est facultative. Uniquement après consentement explicite, un compteur léger envoie le domaine du site, le chemin et le titre de la page ainsi que le fuseau horaire du navigateur à un endpoint Google Apps Script utilisé pour des statistiques agrégées de visites. Le stockage de session évite de compter plus d’une visite par session du navigateur. Aucun suivi publicitaire n’est activé.',
      ...
      de: {
        ...
        privacy_analytics: 'Für die öffentliche Veylto-Labs-Website sind Analytics optional. Erst nach ausdrücklicher Einwilligung sendet ein einfacher Besucherzähler Website-Domain, Seitenpfad, Seitentitel und Browser-Zeitzone an einen Google-Apps-Script-Endpunkt für aggregierte Besuchsstatistiken. Session Storage verhindert, dass mehr als ein Besuch pro Browser-Sitzung gezählt wird. Es ist kein Werbetracking aktiviert.',
